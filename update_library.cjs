...
function applyManualSeasonsSplit(item) {
    ...
    if (episodeCounts.length > 0) {
        ...
        // علامة دائمة تحمي هالعنصر من إعادة الكتابة التلقائية لاحقاً — سكربت التحديث
        // اليومي (refresh_library.js) يتخطى تحديث المواسم لأي عنصر عليه هذي العلامة،
        // عشان ما يمحي تصحيحاً يدوياً ويرجّعه لبيانات TMDB الخاطئة الأصلية.
        item.manual_seasons_override = true;
...
// TMDB يوفر endpoint مستقل لكل موسم فيه تقييم وتاريخ عرض وطاقم خاص بذلك الموسم تحديداً
// (مختلف عن معلومات المسلسل العامة). بدونها، كل المواسم تعرض نفس تقييم/تاريخ/كاست
// المسلسل ككل — وهذا سبب مشكلة "الموسم الثاني ما يتغير فيه شي".
...
// يدمج بيانات كل موسم (تقييم/تاريخ/كاست) داخل مصفوفة seasons — تُستدعى بعد filterValidSeasons.
// الجلب تسلسلي (مو متوازي) عمداً لتخفيف الضغط على حصة TMDB API عند مسلسلات كثيرة المواسم.
...
async function saveMediaItem(currentRunData, globalSeenIds, item, mediaType) {
    ...
    if (details) {
        if (!isTv) {
            ...
            item.seasons = await enrichSeasons(item.id, filterValidSeasons(details.seasons));
        }
                ...
                if (item.poster_path && !globalSeenIds.has(item.id)) {
                    ...
                    if (fullDetails) {
                        if (!isTvShow) {
                            ...
                            item.seasons = await enrichSeasons(item.id, filterValidSeasons(fullDetails.seasons));
                        }
